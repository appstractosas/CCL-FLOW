...
-- MODELO UNIFICADO: una fila por vehículo/LLAVE.
-- Todos los roles (Despachos, Planeación, Portería, Supervisor)
-- trabajan sobre la MISMA tabla.
-- =============================================

-- 1. TRANSPORTES (tabla única de operación)
...
-- 2. CATÁLOGOS (listas desplegables del formulario de nueva llave)
...
-- SEED CLIENTES (ejemplos; el cliente los administra)
...
CREATE INDEX idx_chat_llave ON chat_messages(llave_relacionada);
CREATE INDEX idx_chat_module ON chat_messages(sender_module);
...
-- 4. USUARIOS
-- Nombre, cédula, clave y tipo de usuario (despachador, portero, planeador, supervisor).
-- Cada usuario toma los permisos configurados para su tipo en el módulo ROLES.
CREATE TABLE users (
  id            VARCHAR(50) PRIMARY KEY,
...
-- 5. HISTORIAL DE MOVIMIENTOS POR USUARIO
...
-- =============================================
-- ROW LEVEL SECURITY (acceso de la app)
...
-- NOTAS
-- =============================================
-- 1. El RLS ya queda habilitado con políticas permisivas para que la app funcione
--    con la API anónima. Si necesitas más seguridad, cambia las políticas.
-- 2. La URL correcta de Supabase es: https://gklcxnlseghdqylvnkdb.supabase.co
--    (NO la del dashboard)
