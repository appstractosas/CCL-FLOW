...
export const EstatusBadge: React.FC<{ estado: string }> = ({ estado }) => {
  ...
  return (
    <span className={`${colorClasses} text-[11px] whitespace-nowrap inline-flex items-center gap-1`}>
      • {textoMinuscula}
    </span>
  );
};
...
export const EstadoBadge: React.FC<{ estado: string }> = ({ estado }) => {
  const key = (estado || '').toUpperCase();
  const colorClasses = ESTADO_CLASSES[key] || 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40';
  const textoMayuscula = (estado || '').toUpperCase();

  return (
    <span className={`${colorClasses} border px-2.5 py-0.5 rounded-full font-extrabold text-[11px] whitespace-nowrap uppercase`}>
      • {textoMayuscula}
    </span>
  );
};
