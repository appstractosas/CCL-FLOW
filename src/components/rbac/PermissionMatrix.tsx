    ...
    <div className="bg-[#121726] rounded-2xl border border-zinc-800 overflow-hidden">
              ...
              return (
                <tr key={modId} className="hover:bg-zinc-900/40 transition-colors">
                  <td className="py-2.5 px-5 font-bold text-zinc-200 whitespace-nowrap">
                    {MODULE_LABELS[mod]}
                    <span className="hidden xl:inline text-zinc-600 font-mono text-[10px] ml-2">
                      {modId}
                    </span>
                  </td>
                  {USER_TYPES.map((t) => {
                    ...
                    if (!role)
                      return (
                        <td key={t.value} className="py-2.5 px-4 text-center">
                          —
                        </td>
                      );
      ...
      <div className="px-5 py-3 bg-zinc-900/40 border-t border-zinc-800 flex items-center justify-between gap-3">
        <span className="text-[10px] text-zinc-500">
          Los cambios se guardan automáticamente y quedan registrados en el historial de
          movimientos.
        </span>
